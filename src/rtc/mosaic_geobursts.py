...
def weighted_mosaic(list_rtc, list_nlooks):
    ...
    for i, path_rtc in enumerate(list_rtc):
        ...
        list_dimension[i,:]=(raster_in.RasterYSize,raster_in.RasterXSize)  # Following the numpy convention
    ...
    dim_mosaic=(int(np.ceil((ymin_mosaic-ymax_mosaic)/posting_y)),
                int(np.ceil((xmax_mosaic-xmin_mosaic)/posting_x)))  # Again, following the numpy convention
    ...
    if num_bands==1:
        ...
        #arr_denominator=np.zeros((num_bands,dim_mosaic[0],dim_mosaic[1]))
    ...
    for i, path_rtc in enumerate(list_rtc):
        ...
        if num_bands==1:
            ...
            for i_band in range(num_bands):
                arr_numerator[i_band, offset_imgy:offset_imgy+list_dimension[i,0],
...
if __name__=='__main__':
    ...
    out=weighted_mosaic(list_rtc_vh,list_nlooks_vh)
