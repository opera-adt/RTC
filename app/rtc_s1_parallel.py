...
from rtc_s1 import _update_mosaic_boundaries, _create_raster_obj, _add_output_to_output_metadata_dict, get_radar_grid, _load_parameters
...
def dryrun(cfg: RunConfig, timestamp: str=''):
    '''
    Dryrun geocode burst workflow with user-defined
    args stored in dictionary runconfig `cfg`
    "Dryrun" means that no burst RTC products are not written to file system.
    ...
    for burst_index, (burst_id, burst_pol_dict) in enumerate(cfg.bursts.items()):
        ...
        if len(input_file_list) == 1:
            ...
            pass # NOTE: Temporarily added to keep the if-else statement as it is
        else:
...
def process_runconfig(path_runconfig_burst, path_logfile_burst, full_log_format, timestamp):
    '''
...
def main():
    ...
    timestamp_temp = process_frame_parallel(args)
